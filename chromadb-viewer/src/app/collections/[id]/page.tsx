...
interface CollectionPageProps {
  params: {
    id: string; // This is the collection ID from the URL
  };
}
...
const CollectionPage = async ({ params }: CollectionPageProps) => {
  ...
  const collectionEndpoint = `/api/v2/tenants/${tenantName}/databases/${databaseName}/collections/${collectionId}`;
  ...
  try {
    ...
    if (!res.ok) {
        let errorMessage = `Failed to fetch collection "${collectionId}". Status: ${res.status}`;
        try {
        ...
        try {
            const docRes = await fetch(`${chromaDBUrl}${documentsEndpoint}`, {
                ...
                // Body to get all documents with their metadata and content
                body: JSON.stringify({ include: ["metadatas", "documents"] }),
                cache: 'no-store',
            });
  ...
  return <CollectionDetail collection={collection} documents={documents} documentsError={documentsError} />;
}
